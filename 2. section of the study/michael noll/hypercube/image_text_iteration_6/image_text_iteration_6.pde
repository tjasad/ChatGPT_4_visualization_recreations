// Processing code for two rotating 4D hypercubes (tesseracts) with clockwise vertical rotation and thicker lines
...
void draw() {
  ...
  pushMatrix();
  translate(-250, 0); // Closer together
  rotateX(angle); // Clockwise rotation around a horizontal line (X-axis)
  drawHypercube();
  popMatrix();
  ...
  angle -= 0.01; // Decrement rotation angle for clockwise rotation
}
...
float[] project(float[] v) {
  ...
  // 4D to 3D projection with reduced scale for smaller cubes
  projectedV[0] = x / (distance - w) * 80; // Scale for visibility
  projectedV[1] = y / (distance - w) * 80;
  projectedV[2] = z / (distance - w) * 80;
